...
OFFSET_BASE : float = -.3
CONFIGS : typing.List[str] = ['delay_config', 'packet_loss_config']
...
def data_size_discrete_all_write() -> None:
  ...
  for num_nodes, group_outer in data.groupby('num_nodes'):
    for config, group_med in group_outer.groupby(CONFIGS):
      ...
      pyplot.tight_layout()
      pyplot.savefig(f'plots/data_size-discrete-all_write/data_size/latency/plot-{num_nodes}-{config[0]}-{config[1]}-1.3_2000.0-{TIMESTAMP().strftime(TIMESTAMP_FORMAT)}.png')
    ...
    if len(delay_data) > 0:
      ...
      pyplot.tight_layout()
      pyplot.savefig(f'plots/data_size-discrete-all_write/delay/latency/plot-{num_nodes}-variable-0s-1.3_2000.0-{TIMESTAMP().strftime(TIMESTAMP_FORMAT)}.png')
    ...
    if len(packet_drop_data) > 0:
      ...
      pyplot.tight_layout()
      pyplot.savefig(f'plots/data_size-discrete-all_write/data_size/latency/plot-{num_nodes}-0s-variable-1.3_133.3-{TIMESTAMP().strftime(TIMESTAMP_FORMAT)}.png')
      ...
      pyplot.legend(handles = HARDCODE_LEGEND + HATCH_LEGEND)
      pyplot.tight_layout()
      pyplot.savefig(f'plots/data_size-discrete-all_write/data_size/latency/plot-{num_nodes}-0s-variable-666.7_2000.0-{TIMESTAMP().strftime(TIMESTAMP_FORMAT)}.png')
      ...
      pyplot.tight_layout()
      pyplot.savefig(f'plots/data_size-discrete-all_write/data_size/throughput/plot-{num_nodes}-0s-variable-1.3_133.3-{TIMESTAMP().strftime(TIMESTAMP_FORMAT)}.png')
